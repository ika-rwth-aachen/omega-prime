...
class Metric:
    ...
    def compute_lazy(self, df: pl.LazyFrame, **kwargs) -> tuple[pl.LazyFrame, dict[str,pl.LazyFrame]]:
        try:
            ...
            assert all(p in properties for p in self.computes_properties)
            return df, properties

        except TypeError as e:
            raise TypeError(f'Missing paramter for Metric with compute_func {self.compute_func.__name__}: {repr(e)}') from e
    ...
    def __post_init__(self):
        ...
        self._parameters = [v for k, v in parameters.items() if k not in ['df', 'args', 'kwargs']+self.requires_properties]
...
def metric(computes_columns: list[str]|None = None,
...
@metric(computes_columns = ['distance_traveled', 'vel'])
def driven_distance_and_vel(df) -> tuple[pl.DataFrame, dict[str, pl.DataFrame]]:
...
@metric(requires_columns = ['distance_traveled', 'vel'],
        computes_properties = ['timegaps', 'min_timegaps', 'p_timegaps', 'min_p_timegaps'])
def timegaps_and_p_timegaps(df, /, ego_id, time_buffer=2e9):
...
metrics = [
    timegaps_and_p_timegaps,
    driven_distance_and_vel
]
...
class MetricManager:
    ...
    def __post_init__(self):
        self._dependencies = {val: [i] for i,m in enumerate(self.metrics) for val in [f'column_{n}' for n in m.computes_columns]+[f'property_{n}' for n in m.computes_properties]}|{
            i: [f'column_{n}' for n in m.requires_columns]+[f'property_{n}' for n in m.requires_properties] for i,m in enumerate(self.metrics)
        }

        unresovled_dependencies= {k: v for k,vv in self._dependencies.items() for v in vv if v not in self._dependencies}
        if len(unresovled_dependencies) > 0:
            error_dict = {f'self.metrics[{k}]': v for k,v in unresovled_dependencies.items()}
            raise RuntimeError(f"There are columns and properties required by metrics, that are never computed: {error_dict}")
